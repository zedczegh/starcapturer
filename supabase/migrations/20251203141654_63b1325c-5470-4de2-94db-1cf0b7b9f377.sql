...
DROP FUNCTION IF EXISTS public.get_siqs_time_aggregated_stats(text, text, integer);

CREATE OR REPLACE FUNCTION public.get_siqs_time_aggregated_stats(p_period text DEFAULT 'month'::text, p_source text DEFAULT NULL::text, p_limit integer DEFAULT 100)
 RETURNS TABLE(period_label text, period_start timestamp with time zone, avg_siqs double precision, min_siqs double precision, max_siqs double precision, calculation_count bigint, unique_locations bigint, unique_days bigint, source text)
 LANGUAGE sql
...
AS $function$
  SELECT 
    ...
    COUNT(DISTINCT date_trunc('day', calculated_at)) as unique_days,
