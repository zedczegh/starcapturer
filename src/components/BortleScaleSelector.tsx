...
const BortleScaleSelector: React.FC<BortleScaleSelectorProps> = ({ 
  ...
  // Default to middle value if Bortle scale is unknown
  const sliderValue = bortleScale !== null && bortleScale >= 1 && bortleScale <= 9 
    ? bortleScale 
    : 5;
  
  const getBortleScaleDescription = (value: number | null): string => {
    if (value === null) {
      return t("Unknown: Light pollution data unavailable for this location", "未知：此位置没有可用的光污染数据");
    }
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <Label htmlFor="bortleScale" className="text-sm flex items-center gap-1">
          {t("Light Pollution (Bortle Scale)", "光污染（波特尔尺度）")}
          <Tooltip>
            ...
            <TooltipContent side="right" className="max-w-80">
              {t("The Bortle scale measures the night sky's brightness on a scale of 1-9. Lower values (1-3) indicate darker skies that are better for astrophotography, while higher values (7-9) indicate brighter urban skies with significant light pollution.", 
                "波特尔尺度用1-9的等级测量夜空的亮度。数值越低(1-3)表示天空越暗，越适合天文摄影，而数值越高(7-9)表示城市中有明显光污染的明亮天空。")}
            </TooltipContent>
          </Tooltip>
        </Label>
        <span className="text-sm font-medium">
          {bortleScale !== null ? bortleScale : t("Unknown", "未知")}
        </span>
      </div>
      <Slider
        id="bortleScale"
