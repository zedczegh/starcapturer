...
const SIQSScore: React.FC<SIQSScoreProps> = ({
  ...
  const memoizedValues = useMemo(() => {
    ...
    let interpretation;
    if (displayValue >= 8) interpretation = t("Excellent", "优秀");else if (displayValue >= 6) interpretation = t("Good", "良好");else if (displayValue >= 4) interpretation = t("Average", "一般");else if (displayValue >= 2) interpretation = t("Poor", "较差");else interpretation = t("Bad", "很差");
  ...
  return <motion.div className="mb-4 pb-4 border-b border-cosmic-700/30" initial={{
    opacity: 0,
    y: 15
  }} animate={{
    opacity: 1,
    ...
    duration: 0.5
      ...
      <div className="mt-6 flex justify-center">
        ...
        damping: 15
          ...
          <Link to={`/location/${locationId}`} state={{
          id: locationId,
          name: locationName,
          latitude: latitude,
          longitude: longitude,
          ...
          fromCalculator: true // Add a flag to indicate source
            ...
            <Button size="lg" className="w-full bg-gradient-to-r from-primary/90 to-primary/70 hover:from-primary hover:to-primary shadow-lg hover:shadow-xl transition-all duration-300 group bg-slate-800 hover:bg-slate-700 text-gray-50 py-0 text-sm font-medium">
