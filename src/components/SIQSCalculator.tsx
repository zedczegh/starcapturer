...
const SIQSCalculator: React.FC<SIQSCalculatorProps> = ({
  ...
  const {
    seeingConditions,
    bortleScale
  } = useSIQSAdvancedSettings(parsedLatitude, parsedLongitude);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const updateLocationNameForLanguage = async () => {
      ...
      try {
        const newName = await getLocationNameForCoordinates(lat, lng, language as Language, {
          setCachedData,
          getCachedData
        });
  ...
  const calculateSIQS = useCallback(() => {
    ...
    if (lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
      ...
      calculateSIQSForLocation(lat, lng, locationName, true, localBortleScale, seeingConditions, undefined, setStatusMessage, language as Language).finally(() => {
  ...
  return <motion.div initial="hidden" animate="visible" variants={animationVariants} className="bg-cosmic-800">
      <SIQSCalculatorHeader />
      
      <motion.div variants={animationVariants} transition={{
      delay: 0.1
    }}>
        <StatusMessage message={statusMessage} loading={calculationInProgress} />
      </motion.div>
      
      {siqsScore !== null && <motion.div variants={animationVariants} transition={{
      delay: 0.2
    }}>
          <SIQSScore siqsScore={siqsScore} latitude={parseFloat(latitude)} longitude={parseFloat(longitude)} locationName={locationName} />
        </motion.div>}
      
      <motion.div className="space-y-4" variants={animationVariants} transition={{
      delay: 0.3
    }}>
        <LocationSelector locationName={locationName} loading={loading || calculationInProgress} handleUseCurrentLocation={handleUseCurrentLocation} onSelectLocation={handleLocationSelect} noAutoLocationRequest={noAutoLocationRequest || !shouldAutoRequest} />
        
        {!hideRecommendedPoints && <motion.div variants={animationVariants} transition={{
        delay: 0.4
      }}>
            <RecommendedPhotoPoints onSelectPoint={handleRecommendedPointSelect} userLocation={userLocation} hideEmptyMessage={true} />
          </motion.div>}
      </motion.div>
    </motion.div>;
};
