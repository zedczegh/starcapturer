...
export const calculateNighttimeSIQS = (
  ...
  if (weatherData.nighttimeCloudData && !isNighttimeCloudDataStale(weatherData.nighttimeCloudData)) {
    ...
    nightForecast = filterNighttimeForecast(forecastData.hourly);
  } else {
    ...
    avgCloudCover = calculateAverageValue(nightForecast, 'cloudCover');
  }
  ...
  // Additional weather parameters from night forecast
  ...
  // Add nighttime cloud cover info to the result for reference
  if (weatherData.nighttimeCloudData) {
    siqsResult.nighttimeCloudData = weatherData.nighttimeCloudData;
  }
