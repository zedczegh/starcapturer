...
/**
 * PROTECTED SIQS CORE FUNCTIONS
 * 
 * IMPORTANT: This file contains the core SIQS calculation functions that
 * should not be modified without thorough testing. These functions are
 * critical to the application's functionality.
 * 
 * Any changes to these functions may affect the accuracy of SIQS scores
 * across the entire application.
 */

import { SharedAstroSpot } from '@/lib/api/astroSpots';

// Define a type for SIQS values that can be either a number or object
export type SiqsValue = number | { score: number; isViable: boolean } | any;

/**
 * Core function to safely get a numeric SIQS score from any SIQS format
 * 
 * @param siqs SIQS value which could be a number or object
 * @returns Normalized numeric value of SIQS (0-10 scale)
 */
export function getSiqsScore(siqs?: SiqsValue): number {
  ...
  // Handle objects with siqs property
  if (typeof siqs === 'object' && siqs !== null) {
    // Case: location object with siqs property
 ...
 */
export function formatSiqsScore(siqs?: SiqsValue): string {
...
// Freeze the exported functions to prevent modification
Object.freeze(getSiqsScore);
Object.freeze(normalizeToSiqsScale);
Object.freeze(formatSiqsForDisplay);
Object.freeze(formatSiqsScore);
Object.freeze(isSiqsAtLeast);
Object.freeze(isSiqsGreaterThan);
Object.freeze(sortLocationsBySiqs);
