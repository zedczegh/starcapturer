...
export const useProfileContents = (
  spotId: string, 
  user: boolean, 
  ...
  // Use options to control refresh behavior
  const noRefresh = options?.noRefresh || false;
  
  // Use our smaller hooks with the refresh trigger
  const { spot, isLoading, refetch } = useSpotData(spotId, noRefresh ? 0 : refreshTrigger);
  const { creatorProfile, loadingCreator } = useCreatorProfile(spot?.user_id);
  const { spotImages, loadingImages, refetchImages } = useSpotImages(
    spotId, 
    noRefresh ? 0 : refreshTrigger
  );
  ...
  const triggerRefresh = useCallback(() => {
    if (noRefresh) {
      console.log("Refresh skipped - noRefresh option is enabled");
      return;
    }
  ...
  }, [spotId, lastRefreshTime, noRefresh]);
  ...
  }, [spotId, refetch, fetchComments, refetchImages, lastRefreshTime, noRefresh]);
...
export default useProfileContents;
