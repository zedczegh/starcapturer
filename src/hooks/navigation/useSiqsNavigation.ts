...
export const useSiqsNavigation = (
  ...
  // Update the navigate target when dependencies change
  useEffect(() => {
  ...
  const handleSIQSClick = async (e: React.MouseEvent) => {
    ...
    try {
      // Always use fixed Beijing coordinates to ensure consistency
      const beijing = {
        ...
        if (needsFreshData) {
          ...
          if (freshWeatherData) {
            ...
            const updatedBeijingData = {
              ...beijingData,
              name: beijing.name, // Ensure we set the name explicitly here
            ...
            navigate(`/location/${beijingData.id}`, { 
              state: {
                ...beijingData,
                name: beijing.name // Ensure the name is set here too
              },
              replace: false 
            });
          }
        } else {
          // Use existing data with correct name
          navigate(`/location/${beijingData.id}`, { 
            state: {
              ...beijingData,
              name: beijing.name // Ensure the name is set here
            },
            replace: false 
          });
        }
