...
import { 
  ...
  saveToLocalStorage, 
  cleanupLocalStorage 
} from "@/services/cache/locationCacheService";

// Define the cache expiration time in milliseconds (default: 30 minutes)
const DEFAULT_CACHE_EXPIRATION = CACHE_TTL.WEATHER;
...
export const useLocationDataCache = () => {
  ...
  useEffect(() => {
    const cleanup = () => {
      ...
      // Clean up old localStorage data too
      cleanupLocalStorage();
    };
  ...
  const getCachedData = useCallback((key: string, maxAge: number = DEFAULT_CACHE_EXPIRATION) => {
    ...
    if (!cachedItem) {
      ...
      try {
        const parsedData = getFromLocalStorage(key);
        if (parsedData) {
