...
// Batch processing for SIQS calculation
import { SharedAstroSpot } from "@/lib/api/astroSpots";
import { calculateRealTimeSiqs } from './calculateSiqs';
import { BatchProcessOptions } from './types';
...
export async function batchCalculateSiqs(
  ...
  const { maxParallel = 3, prioritizeDarkSky = true } = options;
  ...
  const prioritizedLocations = prioritizeDarkSky ? 
